fn solve(vec: &[i32]) -> Vec<i32> {
    let mut sortable = vec.to_vec();
    //sort
    sortable.sort();

    //grop
    let mut s = sortable.iter().fold(vec![], |mut a, b| {
        if a.last() == None {
            a.push(vec![b]);
        } else if a.last().unwrap().last().unwrap().eq(&b) {
            //Need local ownership
            let mut x = a.pop().unwrap().to_vec();
            x.push(b);
            a.push(x);
        } else {
            a.push(vec![b]);
    ...
    let l = s.iter().flatten().map(|&&f|f).collect::<Vec<_>>();
    l
}
